...
*Line of best fit formula: ŷ = 13.7 + 0.04x. This means that the estimate for the mean value of emailhr is 14.1*
...
*The model is not suitable, as the data is not continious, but discrete. There is also no clear trend within the plot, as the data is very far spread from the line of best fit.*
...
```{r}
# I chose a logistic model, as the data is categoric, and so a linear model is not suitable.
```
